...
info_data_t g_info = {0};
struct k_mutex info_mutex = Z_MUTEX_INITIALIZER(info_mutex);

/* =========================
 * PARSE INFO
 * ========================= */

static void parse_info(const char *json)
{
	const struct { const char *key; char *dest; } fields[] = {
		{"\"city\":",                g_info.city},
		{"\"country\":",             g_info.country},
		{"\"weather\":",             g_info.weather},
		{"\"temperature_celsius\":", g_info.temperature},
		{"\"latitude\":",            g_info.latitude},
		{"\"longitude\":",           g_info.longitude},
		{"\"unix_epoch\":",          g_info.unix_epoch},
		{NULL, NULL}
	};
	...
	for (int i = 0; fields[i].key != NULL; i++) {
		char *pos = strstr(json, fields[i].key);
		if (!pos) continue;
		...
		if (*pos == '"') {
			pos++;
			while (*pos && *pos != '"' && j < INFO_STR_LEN - 1)
				fields[i].dest[j++] = *pos++;
		} else {
			while (*pos && *pos != ',' && *pos != '}'
			       && *pos != ' ' && j < INFO_STR_LEN - 1)
				fields[i].dest[j++] = *pos++;
		}
		fields[i].dest[j] = '\0';
	}
	...
	LOG_INF("parsed: city='%s' weather='%s' temp='%s' lat='%s' lon='%s' epoch='%s'",
		g_info.city, g_info.weather, g_info.temperature,
		g_info.latitude, g_info.longitude, g_info.unix_epoch);
}
...
static int http_request(const char *path)
{
    ...
    char *raw  = (char *)shared_http_buf;
    char *body = strstr(raw, "\r\n\r\n");
    if (body) {
        body += 4;
        LOG_INF("body: %.200s", body);
        parse_info(body);
    } else {
